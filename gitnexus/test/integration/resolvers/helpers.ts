...
const LEGACY_RESOLVER_PARITY_EXPECTED_FAILURES: Readonly<Record<string, ReadonlySet<string>>> = {
  ...
  gdscript: new Set<string>([
    // GDScript scope-resolution uses registry-primary features that
    // the legacy DAG path does not support:
    // - Built-in type call resolution (Button.new() + signal.connect)
    // - Autoload extraction from .godot resource files
    // - Scene node resolution from .tscn files
    // These require scope-based type binding + registry lookup.
